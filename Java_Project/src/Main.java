...
public class Main {
	...
	public static void main(String[] args) throws UnsupportedAudioFileException, IOException, LineUnavailableException {
		...
		clip.start();														//Müzik başlat
		clip.loop(1);														//Müzik Döngü Açık
		...
		JFrame jf = new JFrame("Restoran Oyunu");							//Pencere İsmi
		jf.setDefaultCloseOperation(jf.EXIT_ON_CLOSE);						//Pencereyi kapatmak programýn sonlandırmasını sağlar.
		jf.setSize(1200, 638);												//Pencere Büyüklüğü
		...
		Staff temp= new Staff(2);											//Çalşan Sayısı
		...
		while(!response.equals("Q")) {
			System.out.println("P = Müzik Oynat, S = Müzik Durdur, R = Müziği Yeniden Başlat");
			...
			switch(response) {
				...
				case ("S"): clip.stop();
				break;
				case ("R"): clip.setMicrosecondPosition(0);					//Müziği yeniden başlatma
				break;
